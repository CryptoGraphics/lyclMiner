...
#ifndef Uint256_INCLUDE_ONCE
#define Uint256_INCLUDE_ONCE
...
public:
    ...
    inline uint256(const basetype& b)
    {
        for (int i = 0; i < WIDTH; i++)
            pn[i] = b.pn[i];
    }

    inline uint256& operator=(const basetype& b)
    {
        for (int i = 0; i < WIDTH; i++)
            pn[i] = b.pn[i];
        return *this;
    ...
    inline uint256& operator=(uint64_t b)
    {
        pn[0] = (unsigned int)b;
        pn[1] = (unsigned int)(b >> 32);
        for (int i = 2; i < WIDTH; i++)
    ...
    inline explicit uint256(const std::string& str)
    {
        SetHex(str);
    }

    inline explicit uint256(const std::vector<unsigned char>& vch)
    {
